...
import type { GitHubBotStatus, IsGitHubBotOption } from './index.ts'
...
cli
  ...
  .action(async (handles: string[], options: IsGitHubBotOption) => {
    ...
    if (handles.length > 1) {
      throw new Error('Only one GitHub account can be checked at a time.')
    }
...
async function getLocalGitHubToken(): Promise<string | undefined> {
  try {
