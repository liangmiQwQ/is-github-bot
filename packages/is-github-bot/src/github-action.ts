...
import type { GitHubBotStatus } from './index.ts'
import { isGitHubBot } from './index.ts'
...
export async function runGitHubAction(input = readActionInput()) {
  ...
  if (input.closeMessage) {
    await createComment(context, item.number, input.closeMessage, status)
  }
...
export function hasHumanFollowUp(item: GitHubActionItem, comments: GitHubComment[]) {
  ...
  return comments.some(comment => {
    if (!comment.user || comment.body?.includes(COMMENT_MARKER)) {
      return false
    }
    if (Date.parse(comment.created_at) <= itemCreatedAt) {
      return false
    }
...
function readActionInput(): GitHubActionInput {
  return {
    ...
    labelBot: readStringEnv('INPUT_LABEL_BOT', 'AI Bot'),
    labelSuspicious: readStringEnv('INPUT_LABEL_SUSPICIOUS', 'Suspicious AI'),
    comment: readBooleanEnv('INPUT_COMMENT'),
    commentMessage: readStringEnv('INPUT_COMMENT_MESSAGE', defaultCommentMessage()),
    close: readBooleanEnv('INPUT_CLOSE'),
    closeAfterDays: readNumberEnv('INPUT_CLOSE_AFTER_DAYS', 3),
    closeMessage: readStringEnv('INPUT_CLOSE_MESSAGE', defaultCloseMessage())
  }
}
