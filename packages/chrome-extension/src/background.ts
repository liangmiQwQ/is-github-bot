import { isGitHubBot, type GitHubBotStatus } from "is-github-bot";
...
async function handleMessage(message: unknown): Promise<ExtensionResponse> {
  if (!isMessage(message)) return { ok: false, error: "Unknown extension message." };

  if (message.type === "get-settings") return { ok: true, settings: await readSettings() };
...
async function checkUser(username: string) {
  ...
  if (cached && Date.now() - cached.checkedAt < FIFTEEN_DAYS_IN_MS) return cached.status;
...
async function closeSpam(context: PageContext, block: boolean) {
  ...
  if (block) await blockUser(context.username);
}
...
async function readCache(): Promise<Record<string, CachedStatus>> {
  const items = await chrome.storage.local.get(CACHE_KEY);
  const value = items[CACHE_KEY];
  if (!value || typeof value !== "object") return {};
  return value as Record<string, CachedStatus>;
}

async function writeCache(cache: Record<string, CachedStatus>) {
  await chrome.storage.local.set({ [CACHE_KEY]: cache });
}

function requireToken(token: string) {
  const normalizedToken = token.trim();
  if (!normalizedToken) throw new Error("Set a GitHub token in extension settings first.");
