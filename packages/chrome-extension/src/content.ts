...
async function run() {
  ...
  if (!settingsResponse.ok || !("settings" in settingsResponse)) return;
...
async function enhanceIssuePage(page: IssuePage, settings: ExtensionSettings) {
  if (shouldSkipAuthor(page.context.username, page.association, settings)) return;
  ...
  if (status === "bot" && settings.showSpamButtons) insertSpamButtons(page.context);
}
...
async function enhanceProfilePage(page: ProfilePage, settings: ExtensionSettings) {
  ...
  if (status === "bot" && settings.showProfileBlockButton) insertProfileBlockButton(page.username);
}
...
function parseReactIssuePage(match: RegExpExecArray): IssuePage | undefined {
  ...
  if (!authorLink || !username || username.endsWith("[bot]")) return undefined;
...
function parseClassicPullRequestPage(match: RegExpExecArray): IssuePage | undefined {
  if (match[3] !== "pull") return undefined;
  ...
  if (!authorLink || !username || authorLink.pathname.startsWith("/apps/")) return undefined;
...
function parseProfilePage(): ProfilePage | undefined {
  ...
  const username =
    document.querySelector<HTMLMetaElement>("meta[property='profile:username']")?.content ??
    document
      .querySelector<HTMLElement>(".p-nickname[itemprop='additionalName']")
      ?.textContent?.trim();
...
async function checkUser(username: string) {
  ...
  if (!response.ok || !("status" in response)) return undefined;
...
function insertSpamButtons(context: PageContext) {
  if (document.querySelector("[data-is-github-bot-actions='issue']")) return;
...
function insertProfileBlockButton(username: string) {
  if (document.querySelector("[data-is-github-bot-actions='profile']")) return;
...
function findAuthorAssociation(authorLink: HTMLElement) {
  const header =
...
function observePage() {
  ...
  const observer = new MutationObserver((mutations) => {
    if (mutations.some(hasObservedNode)) scheduleRun();
  });
